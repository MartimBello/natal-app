...
import { getOrderById, updateOrder, getProducts, deleteOrder } from '@/lib/orders-client';
...
export default function EditOrderPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Initialize quantity inputs from order data
        const initialInputs = new Map<number, string>();
        orderData.products.forEach((product, idx) => {
          initialInputs.set(idx, product.quantity.toString());
        });
        setQuantityInputs(initialInputs);
      } catch (e) {
        setError('Falha ao carregar dados. Certifique-se de que o Supabase está configurado.');
        console.error(e);
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Falha ao atualizar encomenda');
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Falha ao apagar encomenda');
      setDeleting(false);
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-zinc-900 rounded-lg shadow p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-700 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 cursor-not-allowed"
              />
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              onClick={() => router.back()}
              disabled={saving || deleting}
              className="px-4 py-2 border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 rounded-lg font-medium hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Cancelar
            </button>
            {showDeleteConfirm ? (
              <>
                ...
                >
                  {deleting ? 'A apagar...' : 'Confirmar Apagar'}
                </button>
                <button
                  type="button"
